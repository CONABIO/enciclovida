...
def borra_propiedades
  puts "\tBorra propiedades anteriores" if OPTS[:debug]
...
def guarda_relaciones(pez, row)
  6.times do |t|
    ...
    puts "\tGuarda la relacion: #{t}-#{row[t*2].strip}" if OPTS[:debug] if pc.save
  end
end
...
def guarda_propiedades(pez)
  ...
  propiedades.each do |p|
    ...
    pp.propiedad_id = p
    puts "\tGuarda la propiedad: #{p}" if OPTS[:debug] if pp.save
  end
end
