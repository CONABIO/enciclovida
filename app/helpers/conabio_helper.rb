module ConabioHelper
  def taxon_image(photos, params = {})
    ...
    contador=0
    if photos.blank?
      return "<em>No existe en el #{link_to('Banco de Imágenes'.html_safe, 'http://bdi.conabio.gob.mx', :target => :_blank)} una foto asociada con este taxón</em>".html_safe
    end
    ...
    photos.each do |p|
      ...
      params[:size] ||= 'square'
      image_params = {}
      image_params[:alt] = params[:nombre].present? ? params[:nombre] : 'Taxón'.html_safe
