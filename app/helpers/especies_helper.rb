...
<div id=\"biblio_#{biblioCont}\" title=\"Bibliografía\" class=\"biblio\" style=\"display: none\">#{biblio.bibliografia.cita_completa}</div>"
  ...
  def ponCaracteristicaDistribucionAmbienteJS
    ...
    Catalogo::NIVELES_PRIORITARIAS.each do |prio|
      ...
      response[:prioritaria] = response[:prioritaria].to_a << button_tag((image_tag("#{CONFIG.site_url}/assets/app/prioritarias/#{icono}", title: nombre, class: "img-panel", name: "prio_#{prio}")), :class => "btn btn-default btn-xs btn-img-panel btn-prio-#{prio.downcase}", :disabled => '', id: id)
    end
