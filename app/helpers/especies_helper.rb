module EspeciesHelper
  ...
  def enlacesDelArbol(taxon, conClick=nil)     #cuando carga la pagina
    ...
    nodos << "#{link_to("<span class='glyphicon glyphicon-plus' aria-hidden='true' id='span_#{taxon.id}' disabled='true'></span>".html_safe, '', :id =>"link_#{taxon.id}", :class => 'sub_link_taxon btn btn-sm btn-link', :onclick => 'return despliegaOcontrae(this.id);')}"
