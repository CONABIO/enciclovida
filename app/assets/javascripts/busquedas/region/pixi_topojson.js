...
/**
 * Esta funcion solo se manda a llamar desde consola para generar los json transformados,
 * Hay que descomentar la lineas que tengan geojson_data
 * Una vez cargada una capa del mapa (para cada una), ejemplo:
 * download(JSON.stringify(geojson_data), "/tmp/test.json", "application/json")
 * @param {*} data
 * @param {*} filename
 * @param {*} type
 */
...
var despliegaRegiones = function () {
  ...
  PIXI.mesh.MeshRenderer.prototype.render = function render(mesh) {
    ...
    glData.shader.uniforms.translationMatrix = mesh.worldTransform.toArray(
      true
    );
             ...
             //      "application/json"
                  ...
                  interiors.coordinates.forEach(function (arc) {
                    arc.forEach(function (point, index) {
                      ...
                      if (index > 0) {
                        ...
                        var indexFrom =
                          point2index[prevPoint[0] + "#" + prevPoint[1]];
                        if (indexFrom !== indexTo)
                          edges.push([indexTo, indexFrom]);
                      }
                    });
                  });
              ...
              function findFeature(latlng) {
                ...
                for (var i = 0; i < features.length; i++) {
                  ...
                  if (feat.geometry.type === "Polygon") {
                    if (containsPoint(feat.geometry.coordinates, point))
                      return feat;
                  } else {
              ...
              function setSelectedRegion() {
                function findFeatureById(item) {
                  if (
                    item.feature.properties.region_id ==
                    opciones.filtros.region_id
                  )
                    return true;
                  else return false;
                }
                ...
                let feat = tree
                  .all()
                  .find(findFeatureById, opciones.filtros.region_id);
              ...
              utils.getMap().on(
                ...
                L.Util.throttle(function (e) {
                  ...
                  if (feat) {
                    if (focus == undefined) {
                      var x = e.originalEvent.x + 10,
                        y = e.originalEvent.y - 40;
                      $("#nombre-region-hover")
                        .html(feat.properties.nombre_region)
                      ...
                      var region_id =
                        focus._layers[index_layer].feature.properties.region_id;
