...
var soulmateAsigna = function (tipo_busqueda, elem) {
  if (elem == undefined) var elemento = "nombre";
  ...
  var render = function (term, data, type, index, id) {
    if (I18n.locale == "es-cientifico") {
      ...
      if (data.nombre_comun == null) {
        var nombres =
          '<a href="" class="not-active">' + data.nombre_cientifico + "</a>";
      } else {
        var nombres =
          "<b>" +
      ...
      $.each(data.cons_amb_dist, function (i, val) {
        ...
        iconos =
          iconos +
          "<span class='btn-title' tooltip-title='" +
          val +
          "'><i class='" +
          i +
          ev +
      ...
      if (data.geodatos != undefined && data.geodatos.length > 0) {
        iconos =
          iconos +
          "<span class='btn-title' tooltip-title='Tiene datos geográficos'><i class='fa fa-globe'></i></span>";
      }

      if (data.fotos > 0) {
        iconos =
          iconos +
          "<span class='btn-title' tooltip-title='Tiene imágenes'><i class='fa fa-photo'></i><sub>" +
          data.fotos +
          "</sub></span>";
      }

      return (
        foto + " " + nombres + "<h5 class='soulmate-icons'>" + iconos + "</h5>"
      );
    }
  };
  ...
  var select = function (term, data, type) {
    ...
    switch (tipo_busqueda) {
      ...
      case "metamares_proy_esp":
        $("#" + elemento.replace("nombre_cientifico", "especie_id")).attr(
          "value",
          data.id
        );
        break;
      ...
      case "busquedas/explora-por-clasificacion":
        window.location.replace(
          "/explora-por-clasificacion?especie_id=" +
            data.id +
            "&q=" +
            data.nombre_cientifico
        );
      ...
      default:
        ...
        var nom_cientifico = data.nombre_cientifico
          .trim()
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
          .replace(/(^-|-$)/g, "")
...
var soulmateRegionAsigna = function (elem) {
  ...
  var render = function (term, data) {
    ...
    if (data.tipo_region != undefined)
      html += "<sub>" + data.tipo_region + "</sub>";
