...
function anadeLista()
{
    ...
    for (var i=0; i<MAPAS.length; i++)
    {
        ...
        if (i == 0)
        {   //para el default
            x= x+'<li style="list-style: none;"><label><input type="checkbox" onChange="cambiaEstado(this,'+i+')" checked></input>'+MAPAS[i][1]+'</label></li>';
            MAPAS[i][2](MAPAS[i][0]).setMap(map.map);
        } else
            x= x+'<li style="list-style: none;"><label><input type="checkbox" onChange="cambiaEstado(this,'+i+')"></input>'+MAPAS[i][1]+'</label></li>';
    }
    return x;
}
