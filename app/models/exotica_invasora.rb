class ExoticaInvasora < ActiveRecord::Base
  ...
  def guardar_catalogos(tipo, ids)
    ids ||= []
    catalogo_ids = ExoticaCatalogo.where(tipo: tipo, activo: true).pluck(:id)
    exoticas_invasoras_catalogos.where(catalogo_id: catalogo_ids).destroy_all
    ids.reject(&:blank?).uniq.each do |id|
      exoticas_invasoras_catalogos.create!( catalogo_id: id )
    end
  end
end