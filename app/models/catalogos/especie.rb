module EspecieBDILoader
  def self.prepended(base)
    ...
    iucn_path = Rails.root.join('app/lib/web_services/iucn_service.rb')
    
    if File.exist?(bdi_path)
      # Usar Kernel.load para forzar la carga
      Kernel.load bdi_path.to_s
      kernel.load iucn_path.to_s

      Rails.logger.info "✅ BDIService precargado para Especie"
      Rails.logger.info "✅ IUCNService precargado para Especie"

    else
      Rails.logger.error "❌ No se encuentra BDIService en #{bdi_path}"
      Rails.logger.error "❌ No se encuentra IUCNService en #{iucn_path}"
...
begin
  ...
  iucn_paths = [
    Rails.root.join('app/lib/web_services/iucn_service.rb'),
    Rails.root.join('lib/web_services/iucn_service.rb'),
    File.expand_path('../../../lib/web_services/iucn_service.rb', __FILE__)
  ]
  ...
  iucn_paths.each do |path|
    if File.exist?(path)
      begin
        # Usar Kernel.load para forzar la carga
        Kernel.load path.to_s
        Rails.logger.info "[Especie] ✅ IUCNService cargado desde: #{path}"
        bdi_loaded = true
        break
      rescue => e
        Rails.logger.error "[Especie] ❌ Error cargando iucnService: #{e.message}"
      end
    end
  end
  ...
  unless iucn_loaded
    Rails.logger.error "[Especie] ⚠️  IUCNService no pudo ser cargado"
  end
...
class Especie < ActiveRecord::Base
  ...
  attr_accessor :x_estatus, :x_naturalista_id, :x_snib_id, :x_snib_reino, :x_categoria_taxonomica, :x_url_ev, 
                ...
                :x_bibliografia, :x_nombre_autoridad, :x_nombre_autoridad_infraespecie, :x_suprafamilia, :x_prioritaria, :x_prioritaria_conabio, :x_observacion_prioritarias
                 # Para que en el excel sea mas facil la consulta 
  :x_distancia
