class Metamares::AdminController < Metamares::MetamaresController
  ...
  def index
    @usuarios = Usuario.select(:id, :nombre, :apellido, :email).select('nombre_rol').left_joins(:usuario_roles, :roles).where("nombre_rol IN ('AdminMetamares', 'UsuariosMetamares', 'Super usuario')").order(:id).uniq
  end
